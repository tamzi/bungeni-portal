...
class ReportingView(form.PageForm):
    ...
    class IReportingForm(interface.Interface):
        ...
        items = schema.List(title=u'Items to include',
                   required=False,
    ...
    form_fields['items'].custom_widget = horizontalMultiCheckBoxWidget
    ...
    def handle_preview(self, action, data):
        ...
        next_url = ('preview.html?date=' + data['date'].strftime('%Y-%m-%d') + 
            '&doc_type=' + data['doc_type']+'items='+data['items']+'&bill_options='+data['bill_options']+'&agenda_options='+data['agenda_options']+'&motions_options='+data['motions_options']+'&question_options='+data['question_options'] +'&display_minutes=' +
    ...
    def handle_preview(self, action, data):
        ...
        return htmlAgendaReportingView(self.context,self.request).__call__()
        #return (u'<html>\n<head>\n<title>raw</title>\n</head>\n<body>\nThis is a test\n</body>\n</html>')
