...
def setupParliamentWorkflow(self, workflow):
    """Define the ParliamentWorkflow workflow.
    """
    # Add additional roles to portal
    portal = getToolByName(self,'portal_url').getPortalObject()
    data = list(portal.__ac_roles__)
    for role in ['Speaker', 'DeputySpeaker', 'Spokesperson', 'Secretary', 'MemberOfParliament']:
        if not role in data:
            data.append(role)
                ...
                if prm is not None:
                    try:
                        prm.addRole(role, role, 
                                    "Added by product 'Bungeni'/workflow 'ParliamentWorkflow'")
    ...
    workflow.setProperties(title='ParliamentWorkflow')
