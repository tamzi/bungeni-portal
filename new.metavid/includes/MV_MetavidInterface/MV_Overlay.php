...
$smwgShowFactbox=SMW_FACTBOX_HIDDEN;
 ...
 class MV_Overlay extends MV_Component{
	...
	function get_transcript_pages(){
		global $wgUser, $mvgIP, $wgOut, $mvSittingsTable;		
		$sk = $wgUser->getSkin();
		
		
		//$dbr =& wfGetDB(DB_SLAVE);
		//$id = $this->mv_interface->article->mvTitle->getSittingId();
		//$result = $dbr->select($dbr->tableName($mvSittingsTable), '*' , array('id'=>$id));
		//$row = $dbr->fetchObject($result);
		//$this->sitting_start_time = $row->start_time;
		//$this->stream_offset = $row->offset;
		$out='';
	...
	function get_mvd_menu(&$mvd_page){		
		...
		$plink='<a title="'.wfMsg('mv_play').' '.seconds2ntp($mvd_page->start_time) . ' to ' . seconds2ntp($mvd_page->end_time).' " ' .
				...
				'href="javascript:mv_do_play('.$mvd_page->id.');">' .
					...
					seconds2ntp($mvd_page->start_time+ntp2seconds($this->sitting_start_time)+$this->offset) . ' to ' . seconds2ntp($mvd_page->end_time+ntp2seconds($this->sitting_start_time)+$this->offset).'</a>';
