...
function setUpPanels()
{
	...
	for (var i=0; i<headingTags.length; i++)
	{
		...
		else
		{
			...
			panelsStatus[name] = (el.parentNode.className == PANEL_COLLAPSED_CLASS) ? "true" : "false";
		}
...
function changeHeaderText()
{
	...
	for (var i=0; i<headingTags.length; i++)
	{
		...
		if ( el.parentNode.className == PANEL_NORMAL_CLASS ) {
		    el.firstChild.nodeValue = "click here for search options";
		} else if ( el.parentNode.className == PANEL_COLLAPSED_CLASS ) {
		    el.firstChild.nodeValue = "hide search box";
		} else {
		    el.firstChild.nodeValue = "Search Options";
		}
	}
}
...
{ 
    // First change the header text on the h2 header
    changeHeaderText();
