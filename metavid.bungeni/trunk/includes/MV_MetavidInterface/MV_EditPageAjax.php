...
//>>>>>>> .r31965
	...
	function showEditForm( $formCallback=null) {
        ...
        */
        ...
        else if ($editFormType=='answer_en')
        {
            $wgOut->addHTML('<p>Answer to Question Number<select name="smw_Answer_To" id="smw_Answer_To"></select>');
        }
        else if ($editFormType=='anno_en')
        {
...
//cols='{$cols}' {$ew} $hidden>
        ...
        $wgOut->addHTML("<p>Status</p><select name='smw_Status'>");
                foreach($optionsarray as $optionvalue)
                {
                    if ($semantic_data['Status']==$optionvalue)
                    {
                        $wgOut->addHTML("<option selected='selected'>".$optionvalue."</option>");
                    }
                    else
                    {
                        $wgOut->addHTML("<option>".$optionvalue."</option>");
                    }
                }
