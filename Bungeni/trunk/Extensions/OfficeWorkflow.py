...
def setupOfficeWorkflow(self, workflow):
    """Define the OfficeWorkflow workflow.
    """
    # Add additional roles to portal
    portal = getToolByName(self,'portal_url').getPortalObject()
    data = list(portal.__ac_roles__)
    for role in ['ParliamentClerk', 'DeputyParliamentClerk', 'CommitteeClerk', 'ChiefEditor', 'DeputyChiefEditor', 'Editor', 'Reporter']:
        if not role in data:
            data.append(role)
                ...
                if prm is not None:
                    try:
                        prm.addRole(role, role, 
                                    "Added by product 'Bungeni'/workflow 'OfficeWorkflow'")
    ...
    workflow.setProperties(title='OfficeWorkflow')
