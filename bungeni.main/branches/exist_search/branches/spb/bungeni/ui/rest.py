...
class Users(REST):
    ...
    def GET(self):
        if "auth" in self.request.keys():
            ...
            return self.json_response(data)            
                ...
                user_values = users.select([users.c.login ], users.c.active_p == 'A').execute()
                if "user_manager_id" in self.request.keys():
                    plugin_id = self.request["user_manager_id"]
                    data = [dict( id=safeencode(r['login']),
                                  login=safeencode(r['login']), pluginid=plugin_id ) for r in user_values]
                else:
    ...
    def POST(self):
        insert = users.insert()
        insert.values( login = self.request["login"], salt = self.request["salt"],
    ...
    def DELETE(self):
        ...
        users.update().where(users.c.user_id == uid ).values( active_p = 'I').execute()        
...
class enumerateUsers(REST):
    ...
    def GET(self):
        ...
        kw = simplejson.loads(self.request["kw"])
        plugin_id = self.request["user_manager_id"]
