...
class TI(object):
    ...
    @property
    def scope(self):
        # !+CUSTOM_TYPE_DESCRIPTOR until the descriptors for all custom types
        # are actually autogenerated (and will therefore correctly have 
        # descriptor_model.scope="custom" set) the self.custom check below
        # MUST precede the check on self.descriptor_model.scope as otherwise
        # the "in-transit" custom types will not be picked up as custom types.
        if self.custom:
            return "custom"
    ...
    def permission_type_key(self):
        if self.custom:
            ...
            # uses the "group" workflow, that is type-relative reloaded as an
