...
class MV_ManageStaff extends EditPage{
	...
	function displayForm()
	{
		...
		if ($wgUser->isAllowed('managestaff'))
		{
			...
			$html .= $this->getAvailableEditors($sitting_id);
			$html .= $this->getAssignedEditors($sitting_id);
			
			$html .= $this->getAvailableReaders($sitting_id);
			$html .= $this->getAssignedReaders($sitting_id);
			
			$html .= $this->getAvailableReporters($sitting_id);
			$html .= $this->getAssignedReporters($sitting_id);
...
function getAssignedEditors($sitting_id)
	{
		$dbr =& wfGetDB(DB_SLAVE);
...
function getAvailableEditors($sitting_id)
	{
		$dbr =& wfGetDB(DB_SLAVE);
...
function getAssignedReaders($sitting_id)
	{
		$dbr =& wfGetDB(DB_SLAVE);
...
function getAvailableReaders($sitting_id)
	{
		$dbr =& wfGetDB(DB_SLAVE);
...
function getAssignedReporters($sitting_id)
	{
		$dbr =& wfGetDB(DB_SLAVE);
...
function getAvailableReporters($sitting_id)
	{
		$dbr =& wfGetDB(DB_SLAVE);
