...
class MV_Progress{
	...
	public function __construct($article, $status) {
		...
		$this->status = $status;
		$this->article_name = $this->article->getTitle()->getText();
	}
	...
	function execute($p = '') {
		...
		if (!$smwgQEnabled) {
			...
			$wgOut->addHTML('<h1>Progress of '.$this->article->getTitle()->getText().'</h1>');
			$wgOut->addHTML('<div class="progress_links"><a href='.htmlspecialchars($this->article->getTitle()->getLocalURL("action=progress&status=incomplete")).'>Incomplete</a>');
			$wgOut->addHTML('<a href='.htmlspecialchars($this->article->getTitle()->getLocalURL("action=progress&status=complete")).'>Complete</a>');
			$wgOut->addHTML('<a href='.htmlspecialchars($this->article->getTitle()->getLocalURL("action=progress&status=verified")).'>Verified</a>');
			$wgOut->addHTML('<a href='.htmlspecialchars($this->article->getTitle()->getLocalURL("action=progress&status=disputed")).'>Disputed</a>');
			$wgOut->addHTML('<a href='.htmlspecialchars($this->article->getTitle()->getLocalURL("action=progress&status=final")).'>Final</a></div>');
			
			$this->extractQueryParameters($p);
			$this->makeHTMLResult();
			$wgOut->setPageTitle('Progress of '.$this->article_name);
		}
	...
	protected function extractQueryParameters($p) {
		...
		if ($this->status)
			$query_string = '[[Stream::'.$this->article_name.']] [[Status::'.$this->status.']]';
		else
			$query_string = '[[Stream::'.$this->article_name.']] [[Status::Incomplete]]';
	...
	protected function getNavigationBar($res, $urltail) {
		...
		if ($offset > 0) {
			$navigation = '<a href="' . htmlspecialchars($skin->makeSpecialUrl('Progress','offset=' . max(0,$offset-$limit) . '&limit=' . $limit . $urltail)) . '">' . wfMsg('smw_result_prev') . '</a>';
		} else {
		...
		if ($res->hasFurtherResults()) 
			$navigation .= ' <a href="' . htmlspecialchars($skin->makeSpecialUrl('Progress','offset=' . ($offset+$limit) . '&limit=' . $limit . $urltail)) . '">' . wfMsg('smw_result_next') . '</a>';
		...
		foreach (array(20,50,100,250,500) as $l) {
			...
			if ( $limit != $l ) {
				$navigation .= '<a href="' . htmlspecialchars($skin->makeSpecialUrl('Progress','offset=' . $offset . '&limit=' . $l . $urltail)) . '">' . $l . '</a>';
			} else {
				$navigation .= '<b>' . $l . '</b>';
			}
		}
