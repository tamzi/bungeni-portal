...
class MV_Stream {
	...
	function db_load_stream() {
		...
		if($this->name!=''){
			$result = $dbr->select($dbr->tableName($mvStreamTable), '*', array ( 'name' => $this->name));
		}else if($this->id!=''){
			$result = $dbr->select($dbr->tableName($mvStreamTable), '*', array ('id' => $this->id));
		}
	...
	function insertStream($stream_type='') {
		...
		foreach ($this as $key => $val) {
			if($key=='name' || $key=='state' ||$key=='date_start_time' || $key=='duration' || $key=='sitting_id'){
					$insAry[$key] = $val;
			}
		}
		$db = & wfGetDB(DB_WRITE);
