...
function mv_save_staff()
	{
		...
		return MV_Stream::save_staff($_REQUEST['xmldata'], $_REQUEST['stream_id']);
	}	
...
$wgHooks['AddNewAccount'][] = 'customAddNewUser';

function customAddNewUser($user, $byEmail=false) {
	global $wgRequest;
	$role = $wgRequest->getText('mv_role');
	$user->addGroup($role);
	return true ;
}


$wgHooks['SpecialListusersHeader'][] = 'adduserlink';

function adduserlink(&$pager, &$out){
global $wgUser, $wgOut;
$skin = $wgUser->getSkin();
//if( $wgUser->isAllowed('add_user') ){
	$wgOut->addHTML('<div class="admin_links"><a href='.htmlspecialchars($skin->makeSpecialUrl("UserLogin", "type=signup")).'>Add User</a></div>');
//			}
return true;
}
?>
