...
class MV_Stream {
	...
	public static function save_staff($xmldata, $stream_id)
	{
		...
		$result = $dbr->delete($dbr->tableName(stream_assignment), array('stream_id'=>$stream_id));
		foreach($xml->children() as $child)
		{
			...
			{	
				...
				{
				 	$editor_id = $editor['id'];
				 	$sql2 = 'INSERT INTO stream_assignment (user_id, stream_id) VALUES ('.$editor_id.','.$stream_id.')';
			...
			else if ($name=='AssignedReaders')
			{
				...
				{
				 	$reader_id = $reader['id'];
				 	$sql2 = 'INSERT INTO stream_assignment (user_id, stream_id) VALUES ('.$reader_id.','.$stream_id.')';
				 	$result = $dbr->query($sql2);
				}
			}
			else if ($name=='AssignedReporters')
			{
				foreach($child->children() as $reporter)
				{
				 	...
				 	$sql2 = 'INSERT INTO stream_assignment (user_id, stream_id) VALUES ('.$reporter_id.','.$stream_id.')';
				 	$result = $dbr->query($sql2);
				}
			}
		}
