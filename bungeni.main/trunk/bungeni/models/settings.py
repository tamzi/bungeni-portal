...
class SettingsBase(object):
    ...
    def _fetch(self):
        ...
        values = sa.select(
            [setting.c.name, setting.c.value ],
            sa.and_(
                setting.c.propertysheet == self.settings_schema.__name__,
...
def UserSettingFactory(iface):
    ...
    return type("UserSettings%s" % (iface.__name__), (UserSettingsBase,), fields)
...
register.adapter(
    adapts=(interfaces.IBungeniUser,), 
    provides=interfaces.IBungeniUserSettings)(UserSettings)
...
SearchSettings = GlobalSettingFactory(interfaces.ISearchSettings)


@register.utility(provides=interfaces.IBungeniEmailSettings)
...
@register.utility(provides=interfaces.IBungeniRegistrySettings)
class RegistrySettingsUtility(object):
    def __call__(self):
        return RegistrySettings(None)


@register.utility(provides=interfaces.ISearchSettings)
class SearchSettingsUtility(object):
    def __call__(self):
