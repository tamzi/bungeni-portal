 ...
 */
 ...
 //display all MVD category mebers as thubmnails... 
 //display link to rss/playlist
class MvCategoryPage extends CategoryPage{	
...
class MvCategoryViewer extends CategoryViewer {
	...
	function clearCategoryState() {
		...
		if( $this->showGallery ) {
			require_once($mvgIP . '/includes/MV_ImageGallery.php');
			$this->gallery = new MvImageGallery();
	...
	function addMVThumb(Title $title, $sortkey, $pageLength, $isRedirect = false ){
		global $mvgIP;		
		if ( $this->showGallery ) {
			require_once($mvgIP . '/includes/MV_Image.php');
			$image = new MvImage( $title );
			if( $this->flip ) {
	...
	function getImageSection() {
		...
		if( $this->showGallery && ! $this->gallery->isEmpty() ) {
			$title = Title::MakeTitle(NS_SPECIAL, 'MvMediaSearch');			
	...
	function getRssLinks(){
		...
		$sk = $wgUser->getSkin();
		//Special:MvVideoFeed?feed_format=rss&cat=War
		$query = 'feed_format=rss&cat=War';
