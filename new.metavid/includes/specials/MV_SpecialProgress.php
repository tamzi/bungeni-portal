...
class MV_SpecialProgress extends SpecialPage {
	...
	function execute($p = '') {
		...
		if (!$smwgQEnabled) {
			...
			$wgOut->addHTML('<div class="progress_links"><a href='.htmlspecialchars($skin->makeSpecialUrl("Progress","status=incomplete")).'>Incomplete</a>');
			$wgOut->addHTML('<a href='.htmlspecialchars($skin->makeSpecialUrl("Progress","status=complete")).'>Complete</a>');
			$wgOut->addHTML('<a href='.htmlspecialchars($skin->makeSpecialUrl("Progress","status=verified")).'>Verified</a>');
			$wgOut->addHTML('<a href='.htmlspecialchars($skin->makeSpecialUrl("Progress","status=disputed")).'>Disputed</a>');
			$wgOut->addHTML('<a href='.htmlspecialchars($skin->makeSpecialUrl("Progress","status=final")).'>Final</a></div>');
			...
			$this->status=$wgRequest->getText('status');
			$this->getForm();
	...
	protected function extractQueryParameters($p) {
		...
		if ($this->status)
			$query_string = '[[Stream:+]] [[Status::'.$this->status.']]';
		else
			$query_string = '[[Stream:+]]';
		...
		//undesa
		$this->m_params['template'] = 'progress';
		//undesa
